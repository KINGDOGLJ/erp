<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>销货单列表</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  ...
  methods: {
    ...
    async deleteSale(saleId) {
      // 弹框询问用户是否删除数据
      const confirmResult = await this.$confirm('此操作将永久删除该销货单，是否继续?', '提示', {
      ...
      if (!result.success) {
        return this.$message.error('删除销货单失败！')
      }

      this.$message.success('删除销货单成功！')
      this.getPage()
    }
  }
}
